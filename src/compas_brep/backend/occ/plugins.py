...
def make_sweep(profile, path):
    from .factories import occ_sweep
...
def make_pipe(path, radius):
    from .factories import occ_pipe
...
def make_from_curves(curves):
    from .factories import occ_from_curves
...
def make_from_breps(breps):
    from .factories import occ_from_breps
...
def make_from_surface(surface, domain_u=None, domain_v=None):
    from .factories import occ_from_surface
...
def brep_trimmed(brep, plane):
    from .operations import occ_trimmed
...
def brep_split(brep, cutter):
    from .operations import occ_split
...
def brep_slice(brep, plane):
    from .operations import occ_slice
...
def brep_offset(brep, distance):
    from .operations import occ_offset
...
def brep_cap_planar_holes(brep):
    from .operations import occ_cap_planar_holes
...
def brep_overlap(brep_a, brep_b, deflection=None, tolerance=0.0):
    from .operations import occ_overlap
...
def brep_contains(brep, obj):
    from .operations import occ_contains
...
@plugin(category="brep-operations", requires=["OCP"])
def brep_fix(brep):
    from .operations import occ_fix
...
@plugin(category="brep-operations", requires=["OCP"])
def brep_heal(brep):
    from .operations import occ_heal
...
@plugin(category="brep-operations", requires=["OCP"])
def brep_sew(brep):
    from .operations import occ_sew
...
def brep_make_solid(brep):
    from .operations import occ_make_solid
...
def brep_transform(brep, transformation):
    from .operations import occ_transform
...
@plugin(category="brep-operations", requires=["OCP"])
def brep_flip(brep):
    from .operations import occ_flip
...
@plugin(category="brep-operations", requires=["OCP"])
def brep_copy(brep):
    from .operations import occ_copy
...
def brep_volume(brep):
    from .queries import occ_volume
...
def brep_centroid(brep):
    from .queries import occ_centroid
...
def brep_is_solid(brep):
    from .queries import occ_is_solid
...
def brep_extract_topology(brep):
    from .conversion import occ_extract_topology
...
def brep_is_valid(brep):
    from .queries import occ_is_valid
...
def brep_to_data(brep):
    from .conversion import occ_brep_to_data
...
def brep_rebuild(brep, data):
    from .operations import occ_rebuild
