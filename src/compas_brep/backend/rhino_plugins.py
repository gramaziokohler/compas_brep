...
def make_box(box):
    from compas_brep.backend.rhino_backend import make_box as _impl
...
def make_cylinder(cylinder):
    from compas_brep.backend.rhino_backend import make_cylinder as _impl
...
def make_sphere(sphere):
    from compas_brep.backend.rhino_backend import make_sphere as _impl
...
@plugin(category="brep-factories", requires=["Rhino"])
def make_cone(cone):
    from compas_brep.backend.rhino_backend import make_cone as _impl
...
def make_torus(torus):
    from compas_brep.backend.rhino_backend import make_torus as _impl
...
def make_from_mesh(mesh):
    from compas_brep.backend.rhino_backend import make_from_mesh as _impl
...
def make_extrusion(face_or_curve, vector):
    from compas_brep.backend.rhino_backend import make_extrusion as _impl
...
def make_loft(profiles):
    from compas_brep.backend.rhino_backend import make_loft as _impl
...
def make_from_native(native_brep):
    from compas_brep.backend.rhino_backend import from_native as _impl
...
def make_sweep(profile, path):
    from compas_brep.backend.rhino_backend import rhino_sweep
...
def make_pipe(path, radius):
    from compas_brep.backend.rhino_backend import rhino_pipe
...
def make_from_curves(curves):
    from compas_brep.backend.rhino_backend import rhino_from_curves
...
def make_from_breps(breps):
    from compas_brep.backend.rhino_backend import rhino_from_breps
...
def boolean_difference(brep_a, brep_b):
    from compas_brep.backend.rhino_backend import boolean_difference as _impl
...
def boolean_union(brep_a, brep_b):
    from compas_brep.backend.rhino_backend import boolean_union as _impl
...
def boolean_intersection(brep_a, brep_b):
    from compas_brep.backend.rhino_backend import boolean_intersection as _impl
...
def brep_trimmed(brep, plane):
    from compas_brep.backend.rhino_backend import rhino_trimmed
...
def brep_split(brep, cutter):
    from compas_brep.backend.rhino_backend import rhino_split
...
def brep_slice(brep, plane):
    from compas_brep.backend.rhino_backend import rhino_slice
...
def brep_fillet(brep, radius, edges=None):
    from compas_brep.backend.rhino_backend import rhino_fillet
...
def brep_cap_planar_holes(brep):
    from compas_brep.backend.rhino_backend import rhino_cap_planar_holes
...
def brep_contains(brep, obj):
    from compas_brep.backend.rhino_backend import rhino_contains
...
@plugin(category="brep-operations", requires=["Rhino"])
def brep_fix(brep):
    from compas_brep.backend.rhino_backend import rhino_fix
...
def brep_to_step(brep, filepath, **kwargs):
    from compas_brep.backend.rhino_backend import rhino_to_step
...
def brep_from_step(filepath):
    from compas_brep.backend.rhino_backend import rhino_from_step
