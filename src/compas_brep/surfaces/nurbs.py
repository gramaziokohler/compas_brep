...
class NurbsSurface(Data):
    ...
    def aabb(self, n: int = 20) -> Box:
        """Axis-aligned bounding box.

        Parameters
        ----------
        n : int
            Number of sample subdivisions per direction.
    ...
    def obb(self, n: int = 20) -> Box:
        ...
        extents = hi - lo
        obb_center = center + axes.T @ ((lo + hi) / 2)

        xaxis = Vector(float(axes[0, 0]), float(axes[0, 1]), float(axes[0, 2]))
        yaxis = Vector(float(axes[1, 0]), float(axes[1, 1]), float(axes[1, 2]))
        frame = Frame(Point(float(obb_center[0]), float(obb_center[1]), float(obb_center[2])), xaxis, yaxis)
        return Box(float(extents[0]), float(extents[1]), float(extents[2]), frame)

    def _sample_points(self, n: int = 20) -> np.ndarray:
        """Sample (n+1)^2 points on the surface, return (M, 3) array."""
        us = self.space_u(n)
        vs = self.space_v(n)
        pts = []
        for u in us:
    ...
    def intersections_with_line(self, line: Line, n: int = 20) -> list[Point]:
        """Find intersection points of a line with the surface.

        Uses triangle mesh sampling and ray-triangle intersection, then
        optional refinement via Newton iteration.
        ...
        origin = np.array([line.start.x, line.start.y, line.start.z])
        direction = np.array([line.end.x - line.start.x, line.end.y - line.start.y, line.end.z - line.start.z])
        ...
        for i in range(len(us) - 1):
            for j in range(len(vs) - 1):
                ...
                for tri in [
                    ...
                    pt = _ray_triangle_intersect(origin, direction, tri[0], tri[1], tri[2])
                    if pt is not None:
                        hits.append(Point(float(pt[0]), float(pt[1]), float(pt[2])))
        ...
        for p in hits:
            is_dup = False
            for q in unique:
                if (p.x - q.x) ** 2 + (p.y - q.y) ** 2 + (p.z - q.z) ** 2 < 1e-10:
                    is_dup = True
                    break
            if not is_dup:
                unique.append(p)
...
def _ray_triangle_intersect(
    ...
    """Moller-Trumbore ray-triangle intersection. Returns hit point or None."""
    edge1 = v1 - v0
    edge2 = v2 - v0
