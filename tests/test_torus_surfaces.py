...
class TestToroidalSurfaceExtraction:
    def test_torus_face_returns_toroidal_surface(self, torus_face):
        assert isinstance(torus_face.surface, ToroidalSurface)

    def test_torus_radius_axis_correct(self, torus_face):
        assert abs(torus_face.surface.radius_axis - RADIUS_AXIS) <= TOL.absolute

    def test_torus_radius_pipe_correct(self, torus_face):
        assert abs(torus_face.surface.radius_pipe - RADIUS_PIPE) <= TOL.absolute

    def test_torus_frame_is_frame(self, torus_face):
        assert isinstance(torus_face.surface.frame, Frame)

    def test_torus_frame_origin_near_center(self, torus_face):
    ...
    def test_torus_point_at_geometric_accuracy(self, torus_face):
...
class TestBrepFaceTorusAPI:
    def test_surface_type_torus(self, torus_face):
        assert torus_face.surface_type == "torus"

    def test_is_torus_true(self, torus_face):
        assert torus_face.is_torus is True

    def test_is_nurbs_false_for_torus(self, torus_face):
        assert torus_face.is_nurbs is False

    def test_is_planar_false_for_torus(self, torus_face):
        assert torus_face.is_planar is False

    def test_is_cylinder_false_for_torus(self, torus_face):
        assert torus_face.is_cylinder is False

    def test_is_sphere_false_for_torus(self, torus_face):
        assert torus_face.is_sphere is False

    def test_repr_reports_torus(self, torus_face):
...
class TestTorusRoundTrip:
    ...
    def test_restored_face_is_toroidal_surface(self, torus_brep):
        data = torus_brep.__data__
        restored = Brep.__from_data__(data)
        tor_faces = [f for f in restored.faces if isinstance(f.surface, ToroidalSurface)]
        assert len(tor_faces) >= 1

    def test_restored_viewmesh_non_empty(self, torus_brep):
        data = torus_brep.__data__
        restored = Brep.__from_data__(data)
...
class TestAnalyticTorusTessellation:
    def test_tessellate_toroidal_surface(self, torus_face):
    ...
    def test_tessellate_parametric_surface_helper(self, torus_face):
