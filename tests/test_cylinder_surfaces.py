...
def test_cylindrical_surface_extraction_cylinder_face_returns_cylindrical_surface(cylinder_face):
    assert isinstance(cylinder_face.surface, CylindricalSurface)


def test_cylindrical_surface_extraction_cylinder_radius_correct(cylinder_face):
    assert abs(cylinder_face.surface.radius - RADIUS) <= TOL.absolute


def test_cylindrical_surface_extraction_cylinder_frame_is_frame(cylinder_face):
    assert isinstance(cylinder_face.surface.frame, Frame)


def test_cylindrical_surface_extraction_cylinder_frame_origin_near_center(cylinder_face):
    origin = cylinder_face.surface.frame.point
    assert abs(origin.x) < TOL.absolute
    assert abs(origin.y) < TOL.absolute


def test_cylindrical_surface_extraction_cylinder_point_at_geometric_accuracy(cylinder_face):
...
def test_brep_face_cylinder_api_surface_type_cylinder(cylinder_face):
    assert cylinder_face.surface_type == "cylinder"


def test_brep_face_cylinder_api_is_cylinder_true(cylinder_face):
    assert cylinder_face.is_cylinder is True


def test_brep_face_cylinder_api_is_nurbs_false_for_cylinder(cylinder_face):
    assert cylinder_face.is_nurbs is False


def test_brep_face_cylinder_api_is_planar_false_for_cylinder(cylinder_face):
    assert cylinder_face.is_planar is False


def test_brep_face_cylinder_api_repr_reports_cylinder(cylinder_face):
    assert "cylinder" in repr(cylinder_face)


def test_brep_face_cylinder_api_planar_face_surface_type(cylinder_brep):
...
def test_cylinder_round_trip_round_trip_preserves_cylinder_type(cylinder_brep):
    data = cylinder_brep.__data__
    surface_types = [f["surface"]["type"] for f in data["faces"]]
    assert "cylinder" in surface_types


def test_cylinder_round_trip_round_trip_face_count(cylinder_brep):
    data = cylinder_brep.__data__
    restored = Brep.__from_data__(data)
    assert len(restored.faces) == len(cylinder_brep.faces)


def test_cylinder_round_trip_round_trip_volume(cylinder_brep):
    data = cylinder_brep.__data__
    restored = Brep.__from_data__(data)
...
def test_cylinder_round_trip_json_round_trip(cylinder_brep):
    data = cylinder_brep.__data__
...
def test_cylinder_round_trip_restored_face_is_cylindrical_surface(cylinder_brep):
    data = cylinder_brep.__data__
    restored = Brep.__from_data__(data)
...
def test_cylinder_round_trip_restored_viewmesh_non_empty(cylinder_brep):
    data = cylinder_brep.__data__
    restored = Brep.__from_data__(data)
...
def test_analytic_surface_tessellation_tessellate_cylindrical_surface(cylinder_face):
...
def test_analytic_surface_tessellation_tessellate_parametric_surface_helper(cylinder_face):
