...
class TestConicalSurfaceExtraction:
    def test_cone_face_returns_conical_surface(self, cone_face):
        assert isinstance(cone_face.surface, ConicalSurface)

    def test_cone_radius_correct(self, cone_face):
        assert abs(cone_face.surface.radius - RADIUS) <= 1e-6

    def test_cone_height_correct(self, cone_face):
        assert abs(cone_face.surface.height - HEIGHT) <= 1e-6

    def test_cone_frame_is_frame(self, cone_face):
        assert isinstance(cone_face.surface.frame, Frame)

    def test_cone_frame_origin_near_base_center(self, cone_face):
        origin = cone_face.surface.frame.point
        assert abs(origin.x) < 1e-6
        assert abs(origin.y) < 1e-6
        assert abs(origin.z) < 1e-6

    def test_cone_point_at_geometric_accuracy(self, cone_face):
...
class TestBrepFaceConeAPI:
    def test_surface_type_cone(self, cone_face):
        assert cone_face.surface_type == "cone"

    def test_is_cone_true(self, cone_face):
        assert cone_face.is_cone is True

    def test_is_nurbs_false_for_cone(self, cone_face):
        assert cone_face.is_nurbs is False

    def test_is_planar_false_for_cone(self, cone_face):
        assert cone_face.is_planar is False

    def test_is_cylinder_false_for_cone(self, cone_face):
        assert cone_face.is_cylinder is False

    def test_is_sphere_false_for_cone(self, cone_face):
        assert cone_face.is_sphere is False

    def test_is_torus_false_for_cone(self, cone_face):
        assert cone_face.is_torus is False

    def test_repr_reports_cone(self, cone_face):
        assert "cone" in repr(cone_face)

    def test_planar_face_surface_type(self, cone_brep):
...
class TestConeRoundTrip:
    ...
    def test_restored_face_is_conical_surface(self, cone_brep):
        data = cone_brep.__data__
        restored = Brep.__from_data__(data)
        cone_faces = [f for f in restored.faces if isinstance(f.surface, ConicalSurface)]
        assert len(cone_faces) >= 1

    def test_restored_viewmesh_non_empty(self, cone_brep):
        data = cone_brep.__data__
        restored = Brep.__from_data__(data)
...
class TestAnalyticConeTessellation:
    def test_tessellate_conical_surface(self, cone_face):
    ...
    def test_tessellate_parametric_surface_helper(self, cone_face):
