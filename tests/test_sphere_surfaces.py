...
class TestSphericalSurfaceExtraction:
    def test_sphere_face_returns_spherical_surface(self, sphere_face):
        assert isinstance(sphere_face.surface, SphericalSurface)

    def test_sphere_radius_correct(self, sphere_face):
        assert abs(sphere_face.surface.radius - RADIUS) <= TOL.absolute

    def test_sphere_frame_is_frame(self, sphere_face):
        assert isinstance(sphere_face.surface.frame, Frame)

    def test_sphere_frame_origin_near_center(self, sphere_face):
    ...
    def test_sphere_point_at_geometric_accuracy(self, sphere_face):
...
class TestBrepFaceSphereAPI:
    def test_surface_type_sphere(self, sphere_face):
        assert sphere_face.surface_type == "sphere"

    def test_is_sphere_true(self, sphere_face):
        assert sphere_face.is_sphere is True

    def test_is_nurbs_false_for_sphere(self, sphere_face):
        assert sphere_face.is_nurbs is False

    def test_is_planar_false_for_sphere(self, sphere_face):
        assert sphere_face.is_planar is False

    def test_is_cylinder_false_for_sphere(self, sphere_face):
        assert sphere_face.is_cylinder is False

    def test_repr_reports_sphere(self, sphere_face):
...
class TestSphereRoundTrip:
    ...
    def test_restored_face_is_spherical_surface(self, sphere_brep):
        data = sphere_brep.__data__
        restored = Brep.__from_data__(data)
        sph_faces = [f for f in restored.faces if isinstance(f.surface, SphericalSurface)]
        assert len(sph_faces) >= 1

    def test_restored_viewmesh_non_empty(self, sphere_brep):
        data = sphere_brep.__data__
        restored = Brep.__from_data__(data)
...
class TestAnalyticSphereTessellation:
    def test_tessellate_spherical_surface(self, sphere_face):
    ...
    def test_tessellate_parametric_surface_helper(self, sphere_face):
